...
## Architecture
- Target `m5stack-stamps3` (ESP32-S3): **512 KB SRAM, no PSRAM** — heap ~320 KB free at boot (`src/main.cpp:3760`), `<20KB` largest block warns (`src/main.cpp:816`), `<45KB` recycles channel buffers (`src/main.cpp:2893`). Never assume PSRAM; `String` heap fragments quickly and is avoided in hot paths (`src/main.cpp:301,100`). Debug ring is Serial-only when `debug_log_enabled` to save RAM (`src/main.cpp:100`).
- Single source file `src/main.cpp` (~3885 LOC) — all logic in one translation unit.
- Dual FreeRTOS tasks pinned to cores: `NetworkTask` core 0 and `CustomUITask` core 1 (`src/main.cpp:3876`), both under TWDT (10s init, tasks added after create) with `wdt_emergency_flush()`; `loop()` deletes itself (`src/main.cpp:3884`).
- Shared state guarded by `irc_mutex` / `sd_mutex` and `gLogQueue`; 256-byte `log_sector_cache` batches SD writes — respect mutexes when adding SD/IRC access.
- Display: `M5Canvas` sprite `240x109` (~52 KB, fallback `135x214`/`240x135`) is the safe max without PSRAM (`src/main.cpp:3759,1225`); full-screen `240x135` OOMs. Brightness forced to `180` — hardware LED on GPIO 21 is tied to backlight rail (`src/main.cpp:265`).
...
- On-device config: `Fn+O` cycles `SETTINGS → BOUNCER → THEME → WI-FI → CHAT` (`src/main.cpp:1809`); `` ` `` or `Alt+Del` saves & exits via `SD.open("/irc/config.txt", FILE_WRITE)` (`src/main.cpp:1889`). Inside menus: `;` up / `.` down / `,` decrement / `/` increment / `ENTER` toggle (`src/main.cpp:2145,2185,2269`).
  ...
  - `THEME`: `use_light_theme`, `theme_accent`, `text_scale`, `speaker_enabled` (+ volatile `sound_profile`) (`src/main.cpp:1150`)
  - `WI-FI`: `wifi_ssid`/`wifi_pass` + `wifi_ssid2`/`wifi_pass2` (backup shown in UI, `wifi_ssid2` not parsed from `config.txt`) (`src/main.cpp:1176`)
  - Persisted `SD:/irc/config.txt` keys are exactly the 17 parsed in `load_settings_from_sd:342` / written `src/main.cpp:1891`: `wifi_ssid`, `wifi_pass`, `irc_nick`, `bnc_host`, `bnc_port`, `bnc_user`, `bnc_pass`, `channel_log_enabled`, `screen_brightness`, `current_tz_idx`, `use_12_hour_format`, `use_dst`, `use_light_theme`, `theme_accent`, `text_scale`, `speaker_enabled`, `debug_log_enabled`. Repo template `irc/config.txt` / `README.md` list stale keys (`irc_host`, `irc_server_preset`, `proxy_*`, `sasl_*`, `autojoin`, etc.) that are ignored — do not copy them expecting effect.
