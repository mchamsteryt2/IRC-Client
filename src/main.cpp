...
static void appendLog(Tab* tab,const char* raw,const char* serverHHMM=nullptr){
  ...
  if(irc_mutex && xSemaphoreTake(irc_mutex,pdMS_TO_TICKS(50))==pdTRUE){
    if(!SD.exists(gCfg.logRoot)) SD.mkdir(gCfg.logRoot);
    char dirPath[96]; snprintf(dirPath,sizeof(dirPath),"%s/%s",gCfg.logRoot,date); if(!SD.exists(dirPath)) SD.mkdir(dirPath);
    File f=SD.open(path,FILE_APPEND); if(f){ char hhmmss[9]; currentStamp(nullptr,0,hhmmss,sizeof(hhmmss)); f.printf("%s %s\n",hhmmss,raw); f.close(); gGreenBurstMs=millis(); gGreenBurstActive=true; }
    xSemaphoreGive(irc_mutex);
  }
}
...
static void handlePrivmsgC(const char* prefix,const char* target,const char* text,bool isNotice,bool isAction,const char* serverHHMM=nullptr){
  ...
  Tab* dest=nullptr; if(target && isChannelName(target)) dest=getOrCreateTab(target,TAB_CHANNEL); else if(target && eqI(target,gCfg.nick)) dest=getOrCreateTab(nick,TAB_QUERY); else if(target){ if(isChannelName(target)) dest=getOrCreateTab(target,TAB_CHANNEL); else dest=getOrCreateTab(nick,TAB_QUERY); } else dest=findTab("status"); if(!dest) dest=activeTab(); appendLog(dest,disp,serverHHMM); if(isNotice) dest->lines[(dest->head+MAX_LINES_PER_TAB-1)%MAX_LINES_PER_TAB].flags|=0x04;
}
static void handleRawIrc(char* line){
  ...
  if(!strcmp(cmd,"PING")){ const char* tok=argc>0?argv[argc-1]:"cardputer"; char out[64]; snprintf(out,sizeof(out),"PONG :%s",tok); if(gIrcConnected){ WiFiClient* cl=gCfg.useTLS?(WiFiClient*)&gSecure:(WiFiClient*)&gPlain; if(cl&&cl->connected()) cl->print(out), cl->print("\r\n"); } char m[64]; snprintf(m,sizeof(m),"Ping <- %s",tok); logStatus(m); return; }
  ...
  if(!strcmp(cmd,"433")){ char nn[36]; snprintf(nn,sizeof(nn),"%s_",gCfg.nick); safeCopy(gCfg.nick,nn,sizeof(gCfg.nick)); WiFiClient* cl=gCfg.useTLS?(WiFiClient*)&gSecure:(WiFiClient*)&gPlain; if(cl&&cl->connected()){ char out[48]; snprintf(out,sizeof(out),"NICK %s\r\n",gCfg.nick); cl->print(out);} char m[48]; snprintf(m,sizeof(m),"Nick in use, retry %s",gCfg.nick); logStatus(m); return; }
  if(!strcmp(cmd,"JOIN")){
    const char* chan=argc>0?argv[0]:""; char n[32]; nickFromPrefix(prefix?prefix:"",n,sizeof(n));
    if(eqI(n,gCfg.nick)){ Tab* t=getOrCreateTab(chan,TAB_CHANNEL); t->unread=false; t->mention=false; char m[64]; snprintf(m,sizeof(m),"Joined %s",chan); appendLog(t,m,sHHMM); }
  ...
  if(!strcmp(cmd,"PART")){
    ...
    if(eqI(n,gCfg.nick)){ char m[48]; snprintf(m,sizeof(m),"Left %s",chan); if(t) appendLog(t,m,sHHMM); } else { if(t) delNick(t,n); char m[64]; snprintf(m,sizeof(m),"* %s left %s",n,chan); if(t) appendLog(t,m,sHHMM); }
    return;
  }
  ...
  if(!strcmp(cmd,"NICK")){
    char oldn[32]; nickFromPrefix(prefix?prefix:"",oldn,sizeof(oldn)); const char* newn=argc>0?argv[0]:""; if(eqI(oldn,gCfg.nick)) safeCopy(gCfg.nick,newn,sizeof(gCfg.nick));
...
if (gTabCount <= 0 || gTabCount > MAX_TABS || current_tab_index < 0 || current_tab_index >= gTabCount) {
  ...
  if(xSemaphoreTake(irc_mutex,pdMS_TO_TICKS(5))==pdTRUE){
    ...
    if(tab){
      ...
      for(int li=startLogical; li<endLogical; ++li){
        ...
        canvas.setTextColor((cl->flags&0x01)?0xF800:((cl->flags&0x04)?0x8410:0xFFFF),0x0000);
        ...
        if(cl->is_highlight) canvas.drawFastHLine(0,y+ROW_H-1,SCREEN_W,0xFD20);
        if(y+ROW_H > 109) break;
        y+=ROW_H;
      }
    }
    xSemaphoreGive(irc_mutex);
  ...
  // FIXED TOP 12px STATUS BAR directly to hardware glass after pushing sprite
  {
    auto &d=M5Cardputer.Display;
    d.fillRect(0,0,SCREEN_W,12,0x0000);
    d.drawFastHLine(0,11,SCREEN_W,0x8410);
    d.setTextSize(1);
    int x=2;
    for(int i=0;i<gTabCount && i<3 && x<SCREEN_W-2; ++i){
      ...
      for(char* p=tabTok; *p; ++p){
        ...
        else d.setTextColor((*p=='['||*p==']'||*p=='|')?0x8410:0xFFFF,0x0000);
        d.setCursor(x,2); d.print(*p); x+=CHAR_W; if(x>=SCREEN_W-6) break;
      }
      x+=4; if(x>=SCREEN_W) break;
    }
    char audioTag[7]; uint16_t audioCol; if(gCfg.current_audio==1){ safeCopy(audioTag,"[MUTE]",sizeof(audioTag)); audioCol=0xF800; } else { safeCopy(audioTag,"[LOUD]",sizeof(audioTag)); audioCol=0x07E0; }
    ...
    float v=readBatteryVoltage(); int bat_pct=(int)((v-3.2f)/1.0f*100); if(bat_pct<0) bat_pct=0; if(bat_pct>100) bat_pct=100;
    d.setTextColor(0xFFFF,0x0000); d.setCursor(212,2); d.printf("[%d%%]",bat_pct);
  }
  // FIXED BOTTOM 14px INPUT FIELD directly to hardware glass
  {
    auto &d=M5Cardputer.Display;
    d.fillRect(0,INPUT_Y,SCREEN_W,INPUT_H,0x0000);
    d.drawFastHLine(0,INPUT_Y,SCREEN_W,0x8410);
    ...
    if(gInputScroll>0){ for(int fx=0;fx<12;++fx){ uint8_t shade=60+fx*10; uint16_t col=d.color565(shade,shade,shade*0.9); d.drawFastVLine(8+fx,INPUT_Y+1,INPUT_H-2,col); } d.drawFastVLine(20,INPUT_Y+1,INPUT_H-2,0x8410); }
...
static void handleUserInput(const char* in){
  ...
  WiFiClient* cl=gCfg.useTLS?(WiFiClient*)&gSecure:(WiFiClient*)&gPlain;
  auto doSend=[&](const char* fmt, ...){ char buf[IRC_LINE_MAX+1]; va_list ap; va_start(ap,fmt); vsnprintf(buf,sizeof(buf),fmt,ap); va_end(ap); if(cl&&cl->connected()){ cl->print(buf); cl->print("\r\n"); } };
...
void setup(){
  ...
  gSdReady = SD.begin(SD_CS, SPI, 4000000);
  loadConfig();
